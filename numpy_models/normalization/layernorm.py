...
class Layer_Normalization_np:
    ...
    def forward(self, x:np.array, train:bool=True):
        ...
        # dim(x) = [# of batch, feat1*feat2]
        
        #each features mu and var
        self.feature_mu  = np.mean(self.x , axis=1 ).reshape(-1,1)  # [# of batch, 1]
        self.feature_var =  np.var(self.x, axis=1 ).reshape(-1,1)  # [# of batch, 1]

        self.feature_var += self.eps
        self.feature_std = np.sqrt(self.feature_var)
        self.x_minus_mean = self.x - self.feature_mu # [# of batch, # of feat] - [# of batch, 1] (broadcast)
